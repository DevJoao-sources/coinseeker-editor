...
var LEVELS = [[
  ...
  "                                                                                   x",
  "                                                                                   x",
  "                    o         x      xxxxxxxxx     xxxxxxxx     xxxxxxxxxxxxxx     x",
  "                  o   o      xxx     x       x!!!!!x      x!!!!!x            x     x",
  "                             xxx     x       x!!!!!x      x!!!!!x            xx    x",
  "  @                           x      x       x!!!!!x      x!!!!!x            x     x",
...
var playerCoins = 0;
var playerCoinsByLevel = 0;

var playerDeaths = 0;

function updateCoins(i) {
  playerCoinsByLevel++;
  playerCoins++;
  updateScore();
}
 ...
 function updateScore() {
    document.querySelector("#coin").innerHTML = playerCoins + "𝒸";
 }
...
function resetCoinsByLevel() {
  playerCoinsByLevel = 0;
}

function playerDeath() {
  playerCoins -= playerCoinsByLevel;
  resetCoinsByLevel();
  playerDeaths++;
}

function runGame(plans, Display) {
    playerCoins = 0, playerCoinsByLevel = 0, playerDeaths = 0
    ...
    runLevel(new Level(plans[n]), Display, function(status) {
      if (status == "lost") {
        ...
        game_content.innerHTML =
          ...
          playerCoins +
          '</div><div class="text" id="gold">Coins</div></div><div class="item" id="red"><div class="title" id="red">' + playerDeaths + '</div><div class="text" id="red">Mortes</div></div></div><button onclick="startNewGame();">START NEW GAME</button></div>';
      }
    });
  }
  startLevel(0);
